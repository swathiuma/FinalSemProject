...
String query1="select max(cdy_aptno) from cardiology";
String query2="insert into cardiology values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

ps1=con1.prepareStatement("select max(cdy_aptno) from cardiology");
...
for(int i=1,j=6;i<=11;i++,j++)
{
ps2.setString(j,request.getParameter("cdy"+i));
}
int k=ps2.executeUpdate();
if(k>0)
	System.out.println(" successfully uploaded cdy details..");
else
	out.println(" not uploaded cardiac details..");
